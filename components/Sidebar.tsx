...
const Sidebar = ({ isOpen, width, setWidth, standards, standardKey, setStandardKey, auditInfo, setAuditInfo, selectedClauses, setSelectedClauses, onAddNewStandard }: SidebarProps) => {
    ...
    const renderClauseItem = (c: Clause, level: number = 0) => {
        ...
        return (
            <div key={c.id} className={`flex flex-col ${level > 0 ? 'ml-5 border-l-2 dark:border-slate-800' : ''}`}>
                <div className={`group flex items-start gap-3 p-3 rounded-xl transition-all cursor-pointer ${isSelected ? 'bg-indigo-50/80 dark:bg-indigo-900/20' : 'hover:bg-gray-100/50 dark:hover:bg-slate-800/50'}`}>
                    <button onClick={(e) => { e.stopPropagation(); toggleClauseSelection(c); }} className={`mt-1 flex-shrink-0 transition-colors ${selection.some ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-300 dark:text-slate-600 hover:text-gray-400'}`}>
                        {selection.all ? <Icon name="CheckSquare" size={20}/> : selection.some ? <div className="w-5 h-5 bg-indigo-500 rounded flex items-center justify-center"><div className="w-3 h-0.5 bg-white"></div></div> : <Icon name="Square" size={20}/>}
                    </button>
                    <div className="flex-1 min-w-0" onClick={() => hasSubs ? toggleClauseExpand(c.id) : toggleClauseSelection(c)}>
                        {/* Clause Title Row - Increased Font Size */}
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2 flex-wrap">
                                <span className="text-sm font-bold text-indigo-700 dark:text-indigo-400 uppercase shrink-0 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded">{c.code}</span>
                                <span className="text-lg text-slate-900 dark:text-slate-100 font-extrabold tracking-tight leading-tight">{c.title}</span>
                            </div>
                            {hasSubs && <Icon name={isExpanded ? "ChevronUp" : "ChevronDown"} size={16} className="text-gray-400 mt-1 shrink-0"/>}
                        </div>
                        {/* Description Row - Increased Font Size */}
                        <div className="text-base text-slate-600 dark:text-slate-400 italic mt-2 leading-relaxed block w-full whitespace-normal border-t border-gray-100 dark:border-slate-800 pt-2">
                            {c.description}
                        </div>
                    </div>
                </div>
                {hasSubs && isExpanded && <div className="mt-2 flex flex-col">{c.subClauses!.map(sub => renderClauseItem(sub, level + 1))}</div>}
            </div>
        );
    };

    const renderClauses = () => {
        if (!standardKey || !standards[standardKey]) return <div className="text-center text-gray-400 mt-20 italic text-base uppercase tracking-widest opacity-50">Select a Standard</div>;
        const data = standards[standardKey];
        ...
        return data.groups.map(g => {
            ...
            return (
                <div key={g.id} className="mb-4 bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-slate-800/30 cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors" onClick={() => toggleGroupExpand(g.id)}>
                        <div className="flex items-center gap-3">
                            <button className={`p-2 rounded-xl shadow-lg text-white ${getGroupColorClass(g.title)}`}>
                                <Icon name={g.icon} size={20}/>
                            </button>
                            <h4 className="text-sm font-black text-slate-950 dark:text-white uppercase tracking-widest">{g.title}</h4>
                        </div>
                        <Icon name={isGroupExpanded ? "ChevronUp" : "ChevronDown"} size={16} className="text-slate-400"/>
                    </div>
    ...
    // Increased font size for inputs using arbitrary selector classes
    const inputClassOverride = "[&_input]:text-base [&_input]:py-3 [&_select]:text-base [&_select]:py-3";
            ...
            <div className="p-5 border-b border-gray-100 dark:border-slate-800 min-w-[360px] bg-white dark:bg-slate-900">
                <div className="space-y-4">
                    <div className="relative">
                        <IconSelect className={inputClassOverride} icon="Book" iconColor={auditFieldIconColor} value={standardKey} onChange={(e: any) => { if (e.target.value === "ADD_NEW") onAddNewStandard(); else setStandardKey(e.target.value); }} options={[...Object.keys(standards).map(k => ({ value: k, label: standards[k].name })), {value: "ADD_NEW", label: "+ Add New Standard..."}]} defaultText="Select ISO Standard" />
                        {standardKey && (
                    ...
                    <IconSelect className={inputClassOverride} icon="FileEdit" iconColor={auditFieldIconColor} value={auditInfo.type} onChange={(e: any) => setAuditInfo({...auditInfo, type: e.target.value})} options={Object.keys(AUDIT_TYPES).map(key => ({ value: key, label: key }))} defaultText="Select Audit Type" />
                    <div className="grid grid-cols-2 gap-3">
                        <IconInput className={inputClassOverride} icon="Building" iconColor={auditFieldIconColor} placeholder="Company Name" value={auditInfo.company} onChange={(e: any) => setAuditInfo({...auditInfo, company: e.target.value})} />
                        <IconInput className={inputClassOverride} icon="Tag" iconColor={auditFieldIconColor} placeholder="Audit Code (SMO)" value={auditInfo.smo} onChange={(e: any) => setAuditInfo({...auditInfo, smo: e.target.value})} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <IconInput className={inputClassOverride} icon="Users" iconColor={auditFieldIconColor} placeholder="Department" value={auditInfo.department} onChange={(e: any) => setAuditInfo({...auditInfo, department: e.target.value})} />
                        <IconInput className={inputClassOverride} icon="User" iconColor={auditFieldIconColor} placeholder="Auditee/Interviewee" value={auditInfo.interviewee} onChange={(e: any) => setAuditInfo({...auditInfo, interviewee: e.target.value})} />
                    </div>
                </div>
            </div>
            <div className="flex-1 flex flex-col min-h-0 bg-gray-50/40 dark:bg-slate-950/40 min-w-[360px]">
                <div className="p-4 border-b border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm z-10">
                    <IconInput className={`mb-3 ${inputClassOverride}`} icon="AuditUser" iconColor={auditFieldIconColor} placeholder="Lead Auditor Name" value={auditInfo.auditor} onChange={(e: any) => setAuditInfo({...auditInfo, auditor: e.target.value})}/>
                    <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                            <span className="absolute left-3 top-3.5 text-blue-700 dark:text-blue-400 font-bold"><Icon name="Search" size={18}/></span>
                            {/* Increased font size for search input */}
                            <input className="w-full pl-10 pr-4 py-3 text-base bg-gray-100 dark:bg-slate-800 rounded-xl outline-none text-slate-800 dark:text-slate-200 placeholder-gray-400 border border-transparent focus:border-indigo-100" placeholder="Filter clauses..." value={searchQueryRaw} onChange={e => setSearchQueryRaw(e.target.value)}/>
                        </div>
                        {selectedClauses.length > 0 && (
                            <div className="flex gap-2">
                                {/* Restored Copy button */}
                                <button onClick={handleCopySelectedClauses} className="p-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 hover:bg-indigo-100 transition-colors relative shadow-sm border border-indigo-100 dark:border-indigo-900/30" title="Copy Selected Clauses">
                                    <Icon name="Copy" size={20} />
                                </button>
                                {/* Restored Trash button with Badge */}
                                <button onClick={() => setSelectedClauses([])} className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 hover:bg-red-100 transition-colors relative shadow-sm border border-red-100 dark:border-red-900/30" title="Clear Selections">
                                    <Icon name="Trash2" size={20} />
                                    <div className="absolute -top-1 -right-1 bg-red-600 text-white text-[11px] font-black w-5 h-5 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900 shadow-sm">{selectedClauses.length}</div>
                                </button>
                            </div>
                        )}
                    </div>
                 ...
                 <div className="space-y-6">
                    <div className="flex items-center gap-5 p-5 rounded-3xl bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-inner">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 leading-snug mt-1">{health.score === 100 ? 'Perfect! This standard contains all required core clauses for professional auditing.' : 'Warning: Some critical controls or descriptions are missing or incomplete.'}</p>
                        </div>
                    </div>
                    <div className="space-y-3">
                        <h5 className="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Checklist Report</h5>
                        {health.items.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm group hover:border-indigo-100 transition-colors">
                                ...
                                <span className={`text-xs font-black px-3 py-1 rounded-full shadow-sm uppercase ${item.status === 'pass' ? 'text-emerald-700 bg-emerald-100' : 'text-red-700 bg-red-100'}`}>
                                    {item.detail}
                                </span>
                            </div>
                        ))}
