...
export const EvidenceMatrix = forwardRef<EvidenceMatrixHandle, EvidenceMatrixProps>(({
    ...
    // UPDATED: Now we only add rows if they are STRICTLY missing.
    // This allows the Context to handle the primary creation with correct text.
    useEffect(() => {
        setMatrixData(currentMatrix => {
            ...
            selectedClauses.forEach(clauseId => {
                // Only initialize if completely missing (Legacy Sidebar selection support)
                if (!newMatrix[clauseId] || newMatrix[clauseId].length === 0) {
                    ...
                    if (clause) {
                        ...
                        // Basic splitting for legacy mode, usually context already provides this
    ...
    }, [selectedClauses, getClauseById, setMatrixData]); // Keep dependency list minimal
